  ...
  accepts(properties_text) {
    ...
    for (const prop of incoming_properties) {
      if (this.property_table.get(prop.name) !== prop.value) {
        return {ok: false, errors: [`${prop.name} should be ${this.property_table.get(prop.name)}`]};
      }
    }
    if (!ensure_required) {
      return {ok: true};
    ...
    for (const prop_name of this.property_table.keys()) {
      ...
      if (!prop && this.property_table.get(prop_name)) {
        return {ok: false, errors: [`${prop_name} should be ${this.property_table.get(prop_name)}`]};
      }
    }
    return {ok: true};
  }
