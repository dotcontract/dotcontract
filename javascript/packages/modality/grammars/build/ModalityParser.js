      ...
      switch (la_) {
        ...
        case 6:
          ...
          if (
            (_la & ~0x1f) == 0 &&
            ((1 << _la) &
              ((1 << ModalityParser.TRUE) |
                (1 << ModalityParser.FALSE) |
                (1 << ModalityParser.STRING) |
                (1 << ModalityParser.NUMBER) |
                (1 << ModalityParser.PATH))) !==
              0
          ) {
            this.state = 57;
            this.arg();
          ...
          switch (la_) {
            case 1:
              localctx = new OrFormulaContext(
                this,
                new FormulaContext(this, _parentctx, _parentState)
              );
              localctx.left = _prevctx;
              this.pushNewRecursionContext(
                localctx,
                _startState,
                ModalityParser.RULE_formula
              );
              this.state = 110;
      ...
      if (
        !(
          ...
          ((1 << _la) &
            ((1 << ModalityParser.TRUE) |
              (1 << ModalityParser.FALSE) |
              (1 << ModalityParser.STAR) |
              (1 << ModalityParser.NAME))) !==
            0
        )
      ) {
        this._errHandler.recoverInline(this);
