...
export default class DotContract {
    ...
    static async getDCFromDir(dirpath){
        ...
        dc.dotcontract = await DotContractDirectory.fromDir(path.join(dirpath, ".contract"));
        return dc;
    }
    ...
    static create(dotcontract_path, dir = true, genesis = null, config = null) {
        ...
        if(dir){
            dc = new DotContract(TYPE_DIR);
            dc.dotcontract = DotContract.createDCD(path.join(dotcontract_path, ".contract"), genesis, config);
        }
        else{
            dc = new DotContract(TYPE_FILE);
    ...
    {
        await this.dotcontract.reCommit(commitLog, attachments_dir, start_indx, end_indx)
    }
