    ...
    */
    check_always_must_heuristic(modal_formula){
        if(modal_formula.constructor.name === 'GfpFormula'){
            ...
            if(modal_formula.left.constructor.name === 'AndFormula' || modal_formula.right.constructor.name === 'GfpFormula'){
                return this.check_always_must_heuristic(modal_formula.left) && this.check_always_must_heuristic(modal_formula.right)
            }
        }
                ...
                for(const state_name of Object.keys(system.states)){
                    ...
                    for(const [arr_indx, arrow] of system.states[state_name].arrows.entries()){
                        ...
                        if(index!=-1){
                            ...
                            new_arrows.push(arrow);
                            new_arrows[new_arrows.length-1][0] += ' ' + corr_sign + unsigned_prop + ' ';
                        }
